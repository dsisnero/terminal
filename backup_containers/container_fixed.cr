...
# Uses type-safe factory approach instead of runtime reflection
...
module Terminal
  ...
  class Container
    ...
    def register(type : T.class, lifetime : ServiceLifetime = ServiceLifetime::Singleton, &factory : -> T) forall T
      name = type.name
      wrapped_factory = -> { factory.call.as(Object) }
    ...
    def register_instance(type : T.class, instance : T) forall T
      ...
      @mutex.synchronize do
        @instances[name] = instance.as(Object)
      end
      nil
    end
    ...
    def resolve(type : T.class) : T forall T
      ...
      when ServiceLifetime::Transient
        registration.factory.call.as(T)
